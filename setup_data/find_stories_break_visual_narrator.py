...
def find_invalid(load_path):
    ...
    valid_stories = []
    invalid_stories = []
    temp_path = "nlp\\nlp_tools\\visual_narrator\\visual_narrator_extraction\\story_test.txt"
    ...
    for i in range(len(stories)):
        try:
            ...
            with open(temp_path, "w") as file:
                file.write(story)
            command = subprocess.check_output("python nlp\\nlp_tools\\visual_narrator\\run.py nlp\\nlp_tools\\visual_narrator\\visual_narrator_extraction\\story_test.txt -u")
        except:
            sys.tracebacklimit = 0 
