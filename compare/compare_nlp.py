...
def command():
    ...
    Returns:
        ...
        data_type_folder (str): type of data grouping
...
def check_pos_file(load_path, save_name, data_type_folder):
    ...
    if not(load_path.endswith("pos.json")):
        command_input = "python create_pos_baseline.py " + load_path + " " + save_name + " " + data_type_folder
        
        subprocess.run(command_input)

        saving_path = "nlp\\nlp_outputs\\"  + data_type_folder + "\\nlp_outputs_original\\pos_baseline\\" + save_name + "_baseline_pos.json"
...
def bargraph(story_results, x_axis_data, save_folder_path):
    ...
    create_bargraph(persona_precision_data, persona_recall_data, persona_f_measure_data, x_label, persona_title, save_folder_path, "\\persona")
    create_bargraph(entity_precision_data, entity_recall_data, entity_f_measure_data, x_label, entity_title, save_folder_path, "\\entity")
    create_bargraph(action_precision_data, action_recall_data, action_f_measure_data, x_label, action_title, save_folder_path, "\\action")
...
def create_bargraph(precision_data, recall_data, f_measure_data, x_label, title, save_folder_path, save_name):
    ...
    graph.savefig(save_folder_path + save_name + "_bargraph.png")
